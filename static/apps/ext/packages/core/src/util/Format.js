  ...
  return {
    ...
    bpsSize: (function() {
      ...
      return function(size) {
        ...
        if (size < byteLimit) {
          ...
          out = Math.round((size * 10) / byteLimit) / 10 + ' Kbps';
        } else if (size < mbLimit) {
          out = Math.round((size * 10) / kbLimit) / 10 + ' Mbps';
        } else {
          out = Math.round((size * 10) / mbLimit) / 10 + ' Gbps';
        }
        return out;
      };
    })(),
    ...
    number: function(v, formatString) {
      ...
      if (!formatFn) {
        ...
        if (formatString.substr(formatString.length - 2) === '/i') {
          ...
          splitFormat = formatString
            .replace(me.I18NFormatCleanRe, '')
        ...
        if (hasComma) {
          ...
          code[code.length] =
            'thousandSeparator=utilFormat.thousandSeparator;' +
            'thousands.length=0;' +
            'j=fnum.length;' +
            'n=fnum.length%3||3;' +
            'for(i=0;i<j;i+=n){' +
    ...
    plural: function(value, singular, plural) {
      return (
        value +
        ' ' +
        (value === 1 ? singular : plural ? plural : singular + 's')
      );
    },
