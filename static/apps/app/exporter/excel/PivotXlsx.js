...
Ext.define('Ext.exporter.excel.PivotXlsx', {
  ...
  requires: ['Ext.exporter.file.ooxml.Excel'],
  ...
  getContent: function() {
    ...
    if (!matrix || !store || !store.isStore || store.isDestroyed) {
      //<debug>
      Ext.raise(
        'No pivot matrix provided to the exporter or there is no store defined on the matrix'
      );
      //</debug>
      excel.addWorksheet({
        rows: [
          {
            cells:
              'Unable to export the pivot table since no raw data is available'
          }
        ]
      });
      return excel.render();
    ...
    if (matrix && !matrix.calculateAsExcel) {
      Ext.raise(
        'The pivot table calculations are different than what Excel does. Check "calculateAsExcel" config on the pivot matrix!'
      );
    }
    //</debug>
  ...
  generatePivotDataAgg: function(params) {
    ...
    for (i = 0; i < countAgg; i++) {
      ...
      record =
        matrix.results.items.map[
          matrix.grandTotalKey + '/' + matrix.grandTotalKey
        ];
      row2.cells.push({
        ...
        Ext.Array.insert(rows[i].cells, 0, [
          {
            value:
              i === len - 1
                ? this.rowLabelsText
                : i === 0 && countAgg === 1
                ? matrix.aggregate.items[0].excelName
                : null
          }
        ]);
      } else {
        for (j = 0; j < countLeft; j++) {
          if (i === 0 && j === 0 && countAgg === 1) {
            ...
            item = {
              value:
                i === len - 1
                  ? matrix.leftAxis.dimensions.items[j].dataIndex
                  : null
            };
          }
  ...
  generatePivotHeader: function(params) {
    ...
    if (len) {
      params.pivotConfig.colItems = Ext.Array.pluck(
        columns[len - 1],
        'colItem'
      );
  ...
  generatePivotBodyItem: function(params, item) {
    ...
    for (i = 0; i < length; i++) {
      ...
      if (col.aggregate) {
        ...
        layout === 'compact' &&
        col.dataIndex === matrix.compactViewKey
      ) {
        params.styles = params.styles || {};
        level = 'level' + item.level;
        params.styles[level] =
          params.styles[level] ||
          ws.getWorkbook().addCellStyle({
            alignment: {
              horizontal: 'left',
              indent: item.level * 2
            }
  ...
  generatePivotBodyTabularItem: function(params, item) {
    ...
    for (i = 0; i < len; i++) {
      ...
      if (i === 0) {
        rowItem.x.push(
          Ext.Array.indexOf(
            uniqueValues[group.dimension.dataIndex],
            group.value
          )
        );
        for (j = 0; j < length; j++) {
  ...
  generatePivotDataFields: function(params) {
    ...
    for (i = 0; i < countAgg; i++) {
      ...
      if (!aggMap[dimension.aggregator]) {
        Ext.raise(
          'Custom aggregate functions are not supported by this exporter'
        );
      }
      //</debug>
  ...
  generatePivotFields: function(params) {
    ...
    for (i = 0; i < len; i++) {
      ...
      if ((dimension = me.getDimension(matrix.aggregate, field))) {
        ...
        (dimension = me.getDimension(matrix.leftAxis.dimensions, field))
      ) {
        item.axis = 'axisRow';
        if (dimension.getSortable()) {
          item.sortType =
            dimension.direction === 'ASC' ? 'ascending' : 'descending';
        } else {
          item.sortType = 'manual';
        }
      } else if (
        (dimension = me.getDimension(matrix.topAxis.dimensions, field))
      ) {
        item.axis = 'axisCol';
        if (dimension.getSortable()) {
          item.sortType =
  ...
  generatePivotRowFields: function(params) {
    ...
    for (i = 0; i < countLeft; i++) {
      params.pivotConfig.rowFields.push({
        x: Ext.Array.indexOf(
          fields,
          matrix.leftAxis.dimensions.items[i].dataIndex
        )
      });
    }
  },
  ...
  generatePivotColumnFields: function(params) {
    ...
    for (i = 0; i < countTop; i++) {
      params.pivotConfig.colFields.push({
        x: Ext.Array.indexOf(
          fields,
          matrix.topAxis.dimensions.items[i].dataIndex
        )
      });
    }
    if (countAgg > 1) {
  ...
  setupUniqueAggregateNames: function(fields) {
    ...
    for (i = 0; i < len; i++) {
      ...
      for (j = 0; j < length; j++) {
        if (
          String(temp).toLowerCase() === String(fields[j]).toLowerCase() ||
          Ext.Array.indexOf(data, temp) >= 0
        ) {
          temp = name + index;
          index++;
          j = -1;
  ...
  setupUniqueValues: function(dimensions, uniqueValues) {
    ...
    for (i = 0; i < len; i++) {
      item = items[i];
      uniqueValues[item.dataIndex] = Ext.Array.pluck(
        item.values.items,
        'value'
      );
    }
  ...
  generateTopAxisColumns: function(params, colIndex, items) {
    ...
    for (i = 0; i < len; i++) {
      ...
      index = Ext.Array.indexOf(
        uniqueValues[item.dimension.dataIndex],
        item.value
      );
      level = item.level;
