...
Ext.define('Ext.exporter.excel.Xml', {
  ...
  getContent: function() {
    ...
    me.tableHeaderStyleId = me.workbook
      .addStyle(config.tableHeaderStyle)
      .getId();
    me.groupHeaderStyleId = me.workbook
      .addStyle(config.groupHeaderStyle)
      .getId();
    ...
    if (group._summaries && (showSummary || oneLine)) {
      ...
      for (k = 0; k < sLen; k++) {
        ...
        for (j = 0; j < cLen; j++) {
          cell = cells.items[j];
          style = oneLine
            ? me.columnStylesNormal[j]
            : j === 0
            ? styleF
            : me.columnStylesFooter[j];
          row.cells.push({
            value: cell._value,
            styleId: me.getCellStyleId(cell._style, style)
          });
