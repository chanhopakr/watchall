...
Ext.define('Ext.grid.plugin.BaseExporter', {
  ...
  prepareData: function(config) {
    ...
    if (store && store.isBufferedStore) {
      Ext.raise(
        "BufferedStore can't be exported because it doesn't have all data available"
      );
    }
    // </debug>
  ...
  getSummaryRecord: function(collection, columns) {
    ...
    for (i = 0; i < len; i++) {
      colDef = columns[i];
      if (colDef.summary) {
        records = collection.isStore
          ? collection.data.items.slice()
          : collection.items.slice();
        record.set(
          colDef.summaryIndex,
          colDef.summary.calculate(
            records,
            colDef.summaryIndex,
            'data',
            0,
        ...
        record.set(
          colDef.summaryIndex,
          this.getSummary(collection, colDef.summaryType, colDef.summaryIndex)
        );
      }
    }
