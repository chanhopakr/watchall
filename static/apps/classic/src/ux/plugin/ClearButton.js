 ...
 */
Ext.define('apps.ux.plugin.ClearButton', {
  ...
  handleAfterRender: function(textField) {
    this.isTextArea =
      this.textField.inputEl.dom.type.toLowerCase() == 'textarea';
  ...
  createClearButtonEl: function() {
    ...
    this.clearButtonEl = this.textField.inputWrap.createChild({
      tag: 'div',
      cls: this.clearButtonCls
    });
    // zen patched
    // 레이아웃이 테이블일 때 버그 패치
    this.textField.inputWrap.setStyle('position', 'relative');
    // zen patched
   ...
   */
  handleMouseClickOnClearButton: function(event) {
    if (!this.isLeftButton(event)) {
      return;
    }
    this.textField.setValue('');
    this.textField.focus();
    this.textField.fireEvent('afterClear');
  },

  handleMouseOverInputField: function(event) {
  ...
  handleMouseOutOfInputField: function(event) {
  ...
  handleMouseOverClearButton: function(event) {
    event.stopEvent();
  ...
  handleMouseOutOfClearButton: function(event) {
    event.stopEvent();
    if (this.textField.bodyEl.contains(event.getRelatedTarget())) {
  ...
  handleMouseDownOnClearButton: function(event) {
    if (!this.isLeftButton(event)) {
      return;
    }
  ...
  handleMouseUpOnClearButton: function(event) {
    if (!this.isLeftButton(event)) {
      return;
    }
    this.clearButtonEl.removeCls(this.clearButtonCls + '-mouse-down');
  ...
  calculateClearButtonPosition: function(textField) {
    ...
    if (this.textField.triggerWrap) {
      ...
      if (Ext.Object.getSize(this.textField.triggers) > 0) {
        right = 0;
      } else {
        right = 6;
      }
  ...
  isButtonCurrentlyVisible: function() {
    ...
    var cachedVisible = Ext.core.Element.data(
      this.clearButtonEl.dom,
      'isVisible'
    );
  ...
  shouldButtonBeVisible: function() {
    if (
      this.hideClearButtonWhenEmpty &&
      Ext.isEmpty(this.textField.getValue())
    ) {
      return false;
    }
    ...
    //noinspection RedundantIfStatementJS
    if (
      this.hideClearButtonWhenMouseOut &&
  ...
  updateClearButtonVisibility: function() {
    ...
    if (oldVisible != newVisible) {
      if (this.animateClearButton && this.animateWithCss3) {
        this.clearButtonEl.removeCls(
          this.clearButtonCls + (oldVisible ? '-on' : '-off')
        );
        clearButtonEl.addCls(
          this.clearButtonCls + (newVisible ? '-on' : '-off')
        );
      } else {
      ...
      clearButtonEl.setStyle(
        'background-color',
        this.textField.inputEl.getStyle('background-color')
      );
      ...
      if (!(this.isTextArea && Ext.isGecko) && !Ext.isIE) {
        ...
        var deltaPaddingRight =
          clearButtonEl.getWidth() - this.clearButtonEl.getMargin('l');
        var currentPaddingRight = this.textField.inputEl.getPadding('r');
        var factor = newVisible ? +1 : -1;
        this.textField.inputEl.dom.style.paddingRight =
