/**
 * Ext.toolbar.Breadcrumb source Copy 후 extend 를 Ext.form.FieldContainer 로 변경
 *
 ...
 */
Ext.define('apps.toolbar.FieldBreadcrumb', {
  extend: 'Ext.form.FieldContainer',
  xtype: 'fieldbreadcrumb',
  requires: ['Ext.data.TreeStore', 'Ext.button.Split', 'Ext.util.TextMetrics'],
  ...
  updateSelection: function(node, prevNode) {
    var me = this,
      buttons = me._buttons,
      items = [],
      itemCount = Ext.ComponentQuery.query('[isCrumb]', me.getRefItems())
        .length,
    ...
    if (node) {
      ...
      while (currentNode) {
        ...
        width += 60; // add width of arrow icon
        ...
        if (!hasChildNodes) {
          button.addCls('fieldbreadcrumb_leaf');
          width -= 30;
        } else {
          button.removeCls('fieldbreadcrumb_leaf');
        }

        button.setArrowVisible(hasChildNodes);
