Ext.define('apps.ux.grid.FilterBar', {
  extend: 'Ext.AbstractPlugin',
  alias: 'plugin.filterfield',

  require: ['apps.ux.grid.OperatorButton'],
  ...
  init: function(grid) {
    ...
    Ext.Array.each(columns, function(column) {
      ...
      if (!column[me.activateKey]) return true; // Returning false breaks. Wat?

      if (!column.items) Ext.apply(column, { items: [] });
        ...
        triggers: {
          clear: {
            ...
            handler: function() {
              ...
              if (typeof this.clearValue === 'function') this.clearValue();
            }
          }
        },
        listeners: {
          change: {
            fn: function(field) {
              var fn = Ext.isEmpty(field.getValue())
                ? me.clearFilter
                : me.applyFilter;
  ...
  needsOperatorButton: function(column) {
    return (
      column.filter.xtype == 'numberfield' || column.filter.xtype == 'datefield'
    );
  },
  ...
  getDefaultOperator: function(column) {
    if (column.filter.xtype == 'textfield') return 'like';

    if (column.filter.xtype == 'combobox') return 'in';
